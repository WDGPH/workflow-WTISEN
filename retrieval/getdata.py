...
def parse_args():
    ...
    parser.add_argument(
        "--output_file", help="Filename to write output to", required=True, type=str
    )
...
def main(url, report, phu, start, end, output_file, username_file, password_file):
    ...
    logging.info(
        "Browser will refresh each page up to 2 times if initially unavailable"
    )
    ...
    if step1_success:
        ...
        raise RuntimeError(
            "Could not complete step with Selenium within allowed refreshes/wait period"
        )
    ...
    for date_interval in date_intervals:
        logging.info(
            f"Retrieving data released between {date_interval[0]} and {date_interval[1]}"
        )
        ...
        dl_url = "".join(
            [
                ...
                "&prmSelDate=0",
                "&prmStartdate=",
                date_interval[0].strftime("%#m/%d/%Y 00:00:00"),
                "&prmEnddate=",
                date_interval[1].strftime("%#m/%d/%Y 23:59:59"),
        ...
        if response.status_code == 200:
            logging.info(
                f"Response received with Status Code {response.status_code} in {response.elapsed.total_seconds():.2f} seconds"
            )
        else:
            raise RuntimeError(
                f"Response received with Status Code {response.status_code}"
            )

        temp_df = pd.read_csv(
            io.StringIO(response.content.decode("utf-8")), skiprows=3, dtype=str
        )
